"""
This network is derived from vnet2d_v2.
New feature: add attention gated module.
"""
import torch
...
class AttentionGatedModule(nn.Module):
    """ Attention gated module of 2d v-net """

    def __init__(self, gate_channels, in_channels):
        super(AttentionGatedModule, self).__init__()
        self.down_conv = nn.Conv2d(in_channels, in_channels, kernel_size=2, stride=2, groups=1)
        self.in_bn = nn.BatchNorm2d(in_channels)
        self.act = nn.ReLU(inplace=True)
        self.convx1 = nn.Conv2d(in_channels, in_channels, kernel_size=1, stride=1, padding=0)
        self.convg1 = nn.Conv2d(gate_channels, gate_channels, kernel_size=1, stride=1, padding=0)
        self.gate_bn1 = nn.BatchNorm2d(gate_channels)
        self.convg2 = nn.Conv2d(gate_channels, gate_channels // 2, kernel_size=1, stride=1, padding=0)
        self.gate_bn2 = nn.BatchNorm2d(gate_channels // 2)
        self.convxg = nn.Conv2d(in_channels, in_channels, kernel_size=1, stride=1, padding=0)
        self.up_conv = nn.ConvTranspose2d(in_channels, in_channels, kernel_size=2, stride=2, groups=1)
        self.up_bn = nn.BatchNorm2d(in_channels)
        self.out_conv = nn.Conv2d(in_channels, in_channels, kernel_size=1, stride=1, padding=0)
        self.out_bn = nn.BatchNorm2d(in_channels)

    def forward(self, input, skip):
        down=self.down_conv(skip)
        x = self.act(self.in_bn(down))
        x = self.act(self.in_bn(self.convx1(x)))
        g1 = self.act(self.gate_bn1(self.convg1(input)))
        g2 = self.act(self.gate_bn2(self.convg2(g1)))
        xg = self.act(self.in_bn(self.convxg(x + g2)))
        xg = self.act(self.in_bn(self.up_conv(xg)))
        out = self.act(self.out_bn(self.out_conv(xg*skip)))

        return out


class AttentionGatedUpBlock(nn.Module):
    """ Attention gated upsample block of 2d v-net """

    def __init__(self, in_channels, out_channels, num_convs, use_bottle_neck=False):
        super(AttentionGatedUpBlock, self).__init__()
        self.attention_gated_module = AttentionGatedModule(in_channels, out_channels)
        if use_bottle_neck:
            self.rblock = BottResidualBlock2(out_channels, 4, num_convs)
        else:
            self.rblock = ResidualBlock2(out_channels, 3, 1, num_convs)
    ...
    def forward(self, input, skip):
        out = self.attention_gated_module(input, skip)
        out = self.rblock(out)
        return out
...
class VNet2d(nn.Module):
    ...
    def __init__(self, num_in_channels, num_out_channels):
        ...
        self.in_block = InputBlock(num_in_channels, 32)
        self.down_32 = DownBlock(32, 1, use_bottle_neck=False)
        self.down_64 = DownBlock(64, 2, use_bottle_neck=False)
        self.down_128 = DownBlock(128, 3, use_bottle_neck=False)
        self.up_128 = AttentionGatedUpBlock(256, 128, 3, use_bottle_neck=False)
        self.up_64 = AttentionGatedUpBlock(128, 64, 2, use_bottle_neck=False)
        self.up_32 = AttentionGatedUpBlock(64, 32, 1, use_bottle_neck=False)
