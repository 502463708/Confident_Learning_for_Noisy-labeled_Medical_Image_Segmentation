...
class TTestLossV4(nn.Module):
    def __init__(self, beta=0.8, lambda_p=1, lambda_n=0.1):
        super(TTestLossV4, self).__init__()
    ...
    def forward(self, residues, pixel_level_labels, logger=None):
        ...
        if negative_residue_pixels.shape[0] > 0:
            a = negative_residue_pixels.mean().item()
            b = 1 + 0.5 * a
            weights = -0.5 * negative_residue_pixels + b
            mean_residue_pixels_negative = (negative_residue_pixels * weights).mean()
